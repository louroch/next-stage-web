...
import { Menu, X, ChevronLeft, ChevronRight, Play, ArrowLeft } from "lucide-react"
...
export default function HomePage() {
  ...
  const heroTexts = [
    {
      type: "main",
      content: "SOMOS UN BOOKING QUE ENTIENDE LA MÚSICA COMO EXPERIENCIA",
      showButton: false,
    },
    {
      type: "secondary",
      content:
        "No solo representamos talento, lo hacemos crecer. Next Stage nace con una idea simple: que el talento no se quede quieto, que empiece a ocupar el lugar que merece.",
      showButton: true,
    },
  ]
  ...
  const handleMouseDown = (e: React.MouseEvent, carouselRef: React.RefObject<HTMLDivElement>) => {
  ...
  const handleMouseMove = (e: React.MouseEvent, carouselRef: React.RefObject<HTMLDivElement>) => {
  ...
  const handleTouchStart = (e: React.TouchEvent, carouselRef: React.RefObject<HTMLDivElement>) => {
    if (!carouselRef.current) return
    setIsDragging(true)
  ...
  const handleTouchMove = (e: React.TouchEvent, carouselRef: React.RefObject<HTMLDivElement>) => {
    if (!isDragging || !carouselRef.current) return
  ...
  return (
    <div className="min-h-screen bg-[#181313] text-[#D4CFBC] overflow-x-hidden">
      {/* Hero Section */}
      <section className="relative min-h-screen pt-20 md:pt-24 lg:pt-0 lg:h-screen overflow-hidden">
        ...
        >
          <div className="flex items-center justify-between p-1 lg:p-2">
            ...
            {/* Desktop Menu */}
            <div className="hidden lg:flex space-x-8 text-sm font-medium tracking-[0.2em] uppercase">
              <Link
                href="#inicio"
        ...
        {/* Hero Content */}
        <div className="relative z-20 flex items-center min-h-screen px-4 lg:px-12">
          <div className="max-w-7xl mx-auto w-full">
            <div className="relative">
              ...
              {/* Text Carousel */}
              <div className="relative min-h-[200px] lg:min-h-[300px] flex items-center">
                {currentTextIndex === 0 ? (
                  <div className="space-y-6 lg:space-y-8">
                    <div className="relative">
                      <h1 className="text-4xl md:text-5xl lg:text-7xl xl:text-8xl font-black tracking-tight uppercase leading-[0.9] max-w-4xl">
                        SOMOS UN BOOKING QUE ENTIENDE LA MÚSICA COMO EXPERIENCIA
                      </h1>
                      <div className="absolute -bottom-2 left-0 w-32 lg:w-48 h-0.5 bg-[#D4CFBC]"></div>
                    </div>
                  </div>
                ) : (
                  <div className="space-y-6 lg:space-y-8 max-w-4xl">
                    <p className="text-lg md:text-xl lg:text-2xl xl:text-3xl leading-relaxed opacity-90">
                    ...
                    <Button className="bg-[#D4CFBC] text-[#181313] hover:bg-white px-8 py-4 lg:px-12 lg:py-6 text-sm lg:text-base font-medium tracking-widest uppercase rounded-none transition-all duration-300">
                      DESCUBRIR MÁS
                    </Button>
                  </div>
                )}
      ...
      {/* DJs Section */}
      <section id="djs" className="py-16 lg:py-24 px-4 lg:px-12 bg-black/20">
        <div className="max-w-7xl mx-auto">
          <div className="mb-12 lg:mb-20">
            <div className="flex items-center justify-between mb-8">
          ...
          <div className="relative">
            ...
            >
              <div
                className="flex transition-transform duration-700 ease-out lg:grid lg:grid-cols-3 lg:gap-6"
                style={{
                  transform: window.innerWidth >= 1024 ? "none" : `translateX(-${currentDJIndex * 100}%)`,
                }}
              >
                {artists.map((artist, slideIndex) => (
                  <div key={slideIndex} className="w-full flex-shrink-0 px-2 lg:px-0">
                    <div className="max-w-md mx-auto lg:max-w-none">
                      <Link key={artist.slug} href={`/djs/${artist.slug}`} className="group cursor-pointer">
                        <div className="relative overflow-hidden mb-4 lg:mb-6 border border-[#D4CFBC]/20 hover:border-[#D4CFBC] transition-all duration-700">
                          <div className="absolute top-4 left-4 z-10">
                            <span className="text-xs tracking-[0.2em] opacity-70 uppercase">DJS</span>
                          </div>
                          <div className="absolute top-4 right-4 z-10">
                            <span className="text-xs tracking-[0.2em] opacity-70">{artist.id}</span>
                          </div>
                          <Image
                            ...
                            width={300}
                            height={400}
                            className="w-full h-48 md:h-56 lg:h-64 object-cover filter grayscale group-hover:grayscale-0 group-hover:scale-105 transition-all duration-700"
                          />
                          <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity duration-700 flex items-center justify-center">
                            <div className="w-12 h-12 lg:w-16 lg:h-16 border border-[#D4CFBC] rounded-full flex items-center justify-center">
                              <Play className="text-[#D4CFBC] w-4 h-4 lg:w-6 lg:h-6 ml-1" />
                            </div>
                          </div>
                        </div>
                        <div className="text-center">
                          <h3 className="text-lg lg:text-xl font-bold tracking-[0.1em] uppercase group-hover:text-white transition-colors duration-300">
                            {artist.name}
                          </h3>
                        </div>
                      </Link>
            ...
            {artists.length > 1 && (
              <>
                <button
                  onClick={prevDJ}
                  className="absolute left-0 top-1/2 transform -translate-y-1/2 -translate-x-4 lg:hidden bg-transparent border border-[#D4CFBC] text-[#D4CFBC] p-3 hover:bg-[#D4CFBC] hover:text-[#181313] transition-all duration-500"
                >
                  <ChevronLeft size={16} />
                </button>
                <button
                  onClick={nextDJ}
                  className="absolute right-0 top-1/2 transform -translate-y-1/2 translate-x-4 lg:hidden bg-transparent border border-[#D4CFBC] text-[#D4CFBC] p-3 hover:bg-[#D4CFBC] hover:text-[#181313] transition-all duration-500"
                >
      ...
      <section id="equipo" className="py-32 px-4 lg:px-12 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          ...
          <div className="pt-20 border-t border-[#D4CFBC]/20">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
              {[
                { number: "5+", label: "AÑOS DE EXPERIENCIA" },
                { number: "50+", label: "EVENTOS PRODUCIDOS" },
                { number: "15+", label: "ARTISTAS REPRESENTADOS" },
                { number: "3", label: "PROVINCIAS ALCANZADAS" },
              ].map((stat, index) => (
                <div key={index} className="text-center group">
                  <div className="relative mb-6">
                    <div className="text-4xl md:text-6xl font-black tracking-wider mb-2 group-hover:text-[#D4CFBC] transition-colors duration-500">
                      {stat.number}
                    </div>
                    <div className="w-full h-px bg-[#D4CFBC]/30 group-hover:bg-[#D4CFBC] transition-colors duration-500"></div>
                  </div>
                  <div className="text-xs tracking-[0.2em] opacity-70 uppercase leading-relaxed">{stat.label}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* Services Section */}
      <section id="servicios" className="py-24 px-4 lg:px-12">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
            {/* Services for DJs */}
            <div>
              <h3 className="text-2xl font-bold tracking-[0.1em] uppercase mb-8 text-[#D4CFBC]">PARA DJS</h3>
            ...
            <div>
              ...
              <Accordion type="multiple" className="space-y-4">
                <AccordionItem value="talento" className="border-b border-[#D4CFBC]/20">
                  ...
                  <AccordionContent className="pb-6">
                    <p className="text-sm opacity-80 leading-relaxed text-[#D4CFBC]">
                      Representamos una cartera cuidadosamente seleccionada de DJs, garantizando experiencias memorables
